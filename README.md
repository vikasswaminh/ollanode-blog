...
The Astro-powered blog for **OllaNode**, published at **https://blogs.ollanode.com**.
...
- Push to `main` → GitHub Actions on an NH **self-hosted runner** (`nh-marketing`, LXC 505) →
  `npm run build` → `wrangler pages deploy` to the Cloudflare Pages project **`ollanode-blog`** →
  live at `blogs.ollanode.com`.
